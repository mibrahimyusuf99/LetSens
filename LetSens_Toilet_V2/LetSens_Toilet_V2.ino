/*
  ...
  - Sensor Intensitas Cahaya (lux)               -> data DUMMY (simulasi)
...
// simulatePIR() / simulateLight() dengan pembacaan pin ini.
#define PIR_PIN 32     // belum dipakai (masih dummy)
#define LIGHT_PIN 35   // belum dipakai (masih dummy)
...
void setup() {
  ...
  Serial.println();
  Serial.println("================================");
  Serial.println("      ESP32 TOILET MONITOR v2.0");
  Serial.println("================================");
...
// -----------------------------------------------------
// DUMMY: Sensor Intensitas Cahaya (lux)
// Gantilah isi fungsi ini dengan pembacaan sensor BH1750/LDR
// asli (I2C atau analog) saat sudah terpasang.
// -----------------------------------------------------
void simulateLight() {
  // Simulasi lux ruangan toilet antara 20 - 400 lux
  sensorData.lightLux = random(200, 4000) / 10.0;
}
...
<body>
  ...
  <div class="grid">
    ...
    <div class="card">
      <h2>Intensitas Cahaya (dummy)</h2>
      <div class="val"><span id="luxVal">-</span><span class="unit">lux</span></div>
    </div>

    <div class="card">
      <h2>Koneksi WiFi</h2>
      <div class="val" style="font-size:1.1rem" id="ipVal">-</div>
  ...
  <div class="footer">Auto-refresh tiap 3 detik &middot; LetSens Firmware v2.0</div>
...
async function refreshData() {
  try {
    ...
    document.getElementById('luxVal').innerText = d.light_lux.toFixed(1);
